...
def online_lr_update(w, x, y, stepsize=0.1):
    ...
    loss_deriv = logistic_loss_deriv(w.dot(w), y)
    return w - stepsize*loss_deriv*x
...
def main():
    ...
    for name, generator, algo in zip(names, data_generators, updaters):
        ...
        line1, = plt.plot(X[:, 0][positives], X[:, 1][positives], 'ob')

        # TASK 2.3.2
        # plot negatives as red dots
        line2, = plt.plot(X[:, 0][negatives], X[:, 1][negatives], 'or')
